package splash 
{
	...
	public class Splash extends Entity
	{
		...
		public var left:Image = new Image(SPLASH_LEFT);
		public var right:Image = new Image(SPLASH_RIGHT);
		...
		public function Splash(color:uint = 0xFF3366, bgColor:uint = 0x202020, fadeTime:Number = .5, spinTime:Number = 2, spinPause:Number = .5, spins:Number = 720) 
		{
			...
			x = FP.width / 2;
			y = FP.height / 2;
			graphic = new Graphiclist(left, right, cog, lines, fade);
			...
			left.color = color;
			left.smooth = true;
			left.originX = left.width;
			left.originY = left.height / 2;
			left.x -= left.originX + cog.width / 4 + 4;
			left.y -= left.originY;
			leftX = left.x;
			...
			right.color = color;
			right.smooth = true;
			right.originY = right.height / 2;
			right.x += cog.width / 4;
			right.y -= right.originY;
			rightX = right.x;
		...
		override public function update():void 
		{
			...
			left.x = leftX - t * FP.width / 2;
			right.x = rightX + t * FP.width / 2;
			left.scaleY = right.scaleY = tween.scale;
			left.alpha = right.alpha = Ease.cubeIn(tween.scale);
